#Supplementary_code
#'Neighbours, not consumers, drive local intraspecific phytochemical composition in two grassland species'
...
metals <- read.csv("inorganic_nutrients.csv", header=T, stringsAsFactors = T)
metals <- metals %>%
  mutate(total = rowSums(metals[,6:35])) 
metals$Sample <- as.factor(metals$Sample)
metals$Sample <- as.character.factor(metals$Sample)

metalsmeans <- metals %>%
  group_by(Species, Treatment, Diversity) %>%
  summarise(mean(total))

mod1 <- lm(`mean(total)` ~ Species, data=metalsmeans)
anova(mod1)

#Andropogon average 171523.2, Lespedeza average 100940
#Andropogon 1.7 times higher

metalsSi <- metals %>%
  group_by(Species, Treatment, Diversity) %>%
  summarise(mean(Si))

mod2 <- lm(`mean(Si)` ~ Species, data=metalsSi)
anova(mod2)

#Andropogon average 115612.2, Lespedeza average 46926
#Andropogon 2.46 times higher

#Do a PC and see how they change with species and treatments

metals.pca <- prcomp(metals[ ,c(6:35)], scale. = T)
summary(metals.pca)
ggbiplot(metals.pca, alpha=0) + theme_classic()

metalvariables <- metals %>%
  select(Sample, Species, Diversity, Treatment)

metals.pca.axes <- metals.pca$x %>%
  as.data.frame() %>% 
  rownames_to_column(var="Sample")
metals.pca.data <- full_join(metalvariables, metals.pca.axes)
metals.pca.data <- metals.pca.data %>%
  unite('Combo', Diversity:Treatment, remove=F)
meanPC1 <- metals.pca.data %>%
  group_by(Species, Combo) %>%
  summarise(mean(PC1))
meanPC2 <- metals.pca.data %>%
  group_by(Species, Combo) %>%
  summarise(mean(PC2))
meanPCs <- full_join(meanPC1, meanPC2, by=c("Species", "Combo"))

metalsplot <- ggplot(meanPCs, aes(x=`mean(PC1)`, y=`mean(PC2)`, colour=Combo,
                                  shape=Species)) +
  geom_point(size=4) + 
  scale_colour_manual(values=c("#ffb100ff", "#ff8b00ff", "#838383ff", "#3d3d3dff")) +
  scale_shape_manual(values=c(19, 15)) +
  xlab("PC1 (44.1%)") + ylab("PC2 (12.8%)") + 
  theme_classic()
metalsplot
